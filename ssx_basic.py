...
def interferometer(run, calib, scope = '2', diam = 0.1524, showPlot = False,
    writeFiles = False):
    ...
    dv1 = data.signal1 - np.mean(data.signal1[0:50])
    dv2 = data.signal2 - np.mean(data.signal2[0:50])
    ...
    #error = (3.6*10**15)*0.00627*(x1*(k1**-2) + x2*(k2**-2))/np.sqrt(4*((x1/k1)**2 + (x2/k2)**2)-((x1/k1)**2 + (x2/k2)**2)**2)
    return data#,error
...
def interferometer_mod(run, calib, s1=[], s2=[], scope = '2', diam = 0.155575, showPlot = False,
    writeFiles = False):
    """Interferometer analysis routine.
...
def densityclean(path, day, shot, calib_shot, env=[1,1], cutoff=1):
    # Function: This code calibrates density data against a calibration shot, applies a lowpass filter to remove high-frequency noise,
    #           and then corrects for phase error. It must be run on a calibration shot from the day of the measurements in
    #           order to determine offsets/envelope for calibration and the phase error present that day.
    #
    #       Make sure to run the code on a calibration shot (where cos and sin go through a complete cycle!!) before running on shot
    #       with plasma - otherwise, will be unable to find correct phase error/offsets/envelope!!
    #
    #       Depends on mytools.py
    #
    # ----------variables--------
    #day:        mmddyy string corresponding to the day of the measurements
    #shot:       int, shot number
    #calib_shot: boolean, if true, the code will save a .txt file with envelope data, offset of cos/sin data, and the phase error
    #             found for the calibration data, which will be used when analyzing the other shots. If false, the code takes these
    #             values from the saved text file. One text file per day specified
    #env:        array with 2 numbers. The envelope for the data. This is only used when initially loading the calibration data,
    #              for the rest of the data, calibration is based on data taken by this code from the calibration shots
    #cutoff:     float, cutoff frequency for lowpass filter, in units of MHz, 1 MHz corresponds to a period of 1 microsecond

    #Read envelope and minima from files - must process a calibration run before processing others or there won't be a file to read
    if calib_shot == False:
        file = open(path + 'data/2017/' +day+ '/calibration_'+day+'.txt','r')
        offset = [float(file.readline()),float(file.readline())]
        #print 'offset ', offset
        env = [float(file.readline()),float(file.readline())]
        #print 'env ', env
        phasediff = float(file.readline())
        #print 'phasediff ',phasediff
        file.close()

    #Load density data from files
    dens,dens_err = interferometer(day+'r'+str(shot), env, showPlot=False)
    t = dens.time

    #Calibrate cosine and sine data - against itself if there's a calibration shot (assumes the calibration shot goes
    #through mulptiple periods)

    if calib_shot==True:
        #find min and max of data (assumes data goes thru whole period of cos and sin vals)
        offset = [min(dens.signal1),min(dens.signal2)]
        env = [max(dens.signal1)-min(dens.signal1),max(dens.signal2)-min(dens.signal2)]

        #save minima and envelope to file
        file = open("calibration_"+day+".txt","w")
        file.write(str(offset[0])+'\n') #first offset
        file.write(str(offset[1])+'\n') #second offset
        file.write(str(env[0])+'\n') # first envelope
        file.write(str(env[1])+'\n') # second envelope
        file.close()

        print ('Found envelope of '+str(env)+' and offsets of '+str(offset))

    #calibrate data
    s1 = (dens.signal1-offset[0])/env[0]
    s2 = (dens.signal2-offset[1])/env[1]

    def lowpass_gfilter(t,signal,fc,return_mean=True):

        dtt = t[1]-t[0]
        sigmat = 1./(2*np.pi*fc) # width of Gaussian smoothing kernel [s]
        sigmap = np.round(sigmat/dtt)# width of gaussian smoothing kernel [points]
    ...
    #Apply lowpass filter to calibrated cosine and sine data
    s1_clean_phaseerror = lowpass_gfilter(t,s1,cutoff,return_mean=True) #intermediate step between lowpass and fixing phase error
    s2_clean = lowpass_gfilter(t,s2,cutoff,return_mean=True)

    #circlify by removing phase error found from calibration
    #correct for phase shift in cosine data

    if calib_shot == True:
        #find point where y is at maximum i.e. sine is = 1 so phi = pi/2 -> x = cos(pi/2 + error in phase separation)
        x_of_ymax = [] #array of xvals around the ymax point
        for i in range(len(s2_clean)):
            #add high-y vals to list - going to average x values
            if s2_clean[i] > 0.98:
                x_of_ymax.append(s1_clean_phaseerror[i])

        x_ym = np.median(x_of_ymax)
        phasediff = np.arccos((x_ym-0.5)*2)-np.pi/2
        print ('Found phase error of ',phasediff*180/np.pi)

        #log the phase error for this day
        file = open("calibration_"+day+".txt","a")
        file.write(str(phasediff)+'\n') #phase offset
        file.close()

    #create a new array for the final signal
    s1_clean = [0]*len(s1_clean_phaseerror)

    #remove effects of phase error
    for i in range(len(s1_clean_phaseerror)):
        s1_clean[i] = ((s1_clean_phaseerror[i]*2-1)/np.cos(phasediff)+(s2_clean[i]*2-1)*np.tan(phasediff))*0.5+0.5
