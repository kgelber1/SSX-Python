import numpy as np
import MagBField_Plotter_Luke
# reload(MagBField_Plotter_Luke)
import process_mjmag_data as mj
# reload(mj)
import iter_smooth as ism
import matplotlib.pylab as plt
import mytools as my
import ssx_data_read_basic as sdr
import process_mag_data_4x4 as pmd


### Luke's code
##############################################
#shot = '020618r2'


day = '061219'#'062618'#'101917'
shot = 13#85
shot = day+'r'+ str(shot) #'40'#

# time,bdot,timeb,b,bmod = mj.process_mjmag_data(day+'r'+str(shot))
time,timeb,b,bmod, data = pmd.get_Bxy_vecRendering_format(shot)
timeb=timeb-timeb[0]-2

######### fixing bounds using a function called fix_bounds ###########
t0 = 20
tf = 70
t_index, t = my.fix_bounds(np.array((t0, tf)), timeb)
t0 = t_index[0]
tf = t_index[-1]
N = 16
sample_Freq = 10 # sampling frequency
b = b[:,:,t0:tf]
s = np.arange(N) * 1.5# - 2.86
x = b[0,0:N,:].T[0:-1:sample_Freq]
y = b[1,0:N,:].T[0:-1:sample_Freq]
#z = b[2,0:20,:].T[0:-1:sample_Freq] # Bz is not called by the function being used
t = timeb[t0:tf][0:-1:sample_Freq]
#path = 'C:\\Users\\Manjit\\Documents\\Python_Scripts\\Basic_Package\\Data\\2017\\Animation\\'+shot

path = 'Data\\2018\\'+day+'\\'+day+'-Analysed\\'

##############################################
######## Plotting the magnetic field vector rendering ############

#Bmag = MagBField_Plotter_Luke.BField_mag_Animator(shot,t,s,x,y) #Instantiate the BField_xy_Animator Object
Bmag = MagBField_Plotter_Luke.BField_mag_Animator(shot,t,s,x,y)
#Bmag.gen_Arrows(325) #Plot the vectors at some time, in this case t[10]
animation = Bmag.make_animation() #Now create an animation
Bmag.save_animation(path,animation) #Save the animation

plt.show()
