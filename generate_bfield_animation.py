...
# plt.rcParams['animation.ffmpeg_path'] = 'C:\\Program Files\\ffmpeg\\ffmpeg.exe'
"""
This one creates an animation of JUST the B-field
...
class BField_mag_Animator(object):
    ...
    def __init__(self,title,t,probe_locs,x,y,z, num_probes = 16):
        ...
        #to force the axis to be equal, first find which one has the biggest scale:
        x_min = np.amin(x)
        y_min = np.amin(y)
        z_min = np.amin(z)
...
# animation function.  This is called sequentially
    ...
    def make_animation(self):
        ...
        #self.gen_Arrows(0) #if you want to generate a frame at a certain point
        fig = self.fig
    ...
    def save_animation(self,path,anim):
        ...
        writ = animation.FFMpegFileWriter(fps= 30)
        filename = path + 'B_VecRendering_'+self.title +'.mp4'
        anim.save(filename,writer=writ)
        # Writer = animation.FFMpegWriter(fps=30, codec='libx264')
        # anim._animation.save("./tensorflowvisu_video.mp4", writer=mywriter)
...
def main():
    ...
    path = os.getcwd() + '\\data\\2019\\' +day+'\\Anayzed\\'
