...
def peakDetect(y):
    ...
    q = where(y > 0.1 * ymax)
#    q = where(y > 0.1 * ymax)
#Manjit: Above line was original where it will count all the peaks which will
#fall within 10% of the maximum.
    q = q[0]
    if len(q) == 0:
        p = 0
        return p
...
class ids(sdr.dtac_diag):
    ...
    def __init__(self, shotname, **kw):
        ...
        self.order = 25
        self.bintime = 1
        #bintime: number of micro-seconds you want to sum over.
        # Default is 1 (i.e., no summing).
        self.voltage = 800
        ...
        if not(hasattr(self.delays, 'type')):
            self.delays = None
            self.delay_channel = None
            self.delay = ['T', 0.1]
        elif not self.delays.type:
            self.delays = None
            self.delay_channel = None
            self.delay = ['T', 0.1]
    ...
    def _IDSfitGaussian(self, i, mu = None, sigma = None):
        ...
        offset = 0
        p0 = (mu, sigma, scale, offset)
        # -----Modification by David--------
        #print i, p0
        #for j in np.arange(16):
        #     if self.data[j,i] < 30:
        #       self.data[j,i]=1.0
        # -----Modification by Manjit--------
        #print i, p0
#         for j in np.arange(16):
#             for i in range(3):
#                 self.data[j,i]=scipy.signal.detrend(self.data[j,i])
             #self.data[j,i]=1.0
        pfit, cov, chi2, out['success'] = mf.curveFit(mf.pOffsetGaussian, x[q],
        ...
        if out['success'] == 1 and (type(cov) == type(empty(0))):
            ...
            out['xfit'] = arange(x[q[0]], x[q[-1]])
            out['yfit'] = mf.pOffsetGaussian(out['xfit'], *pfit)
        else:
            v = [0] * 3
            vErr = v
    ...
    def im(self, timerange = [20, 80], plotLog = False, interp = 'nearest',
        ...
        """Plots all IDS channels as a 2D array image.
    ...
    def plotRawIDStp(self, ts):
        ...
        fig.clear()
        numChannels = self.data.shape[0]
        plot(self.data[:,tp], 'bo', color = 'k', markersize = 8)#, markeredgewidth=2))
        ...
        ylabel('No. of photons')
        xlabel('PMT channel')
		  #setp(axis.spines.values(), linewidth=2)#changing the axis linewidth
		  #axis.tick_params(axis='both', direction='in', length=7, width =2, labelsize = 20)
		  #ylabel('photons')
        #xlabel('channel')
    ...
    def processIDS(self, dv0 = 0, tracescale = 1, ts = None, plotErr = False,
        ...
        maybeThreshLo = 0.1
        #maybeThreshLo = .1
        #maybeThresh = 0.5 # is the original value in Tim's code
        ...
        tqDouble2 = where((chi2 < 0.5 * chi1), 1, 0)
        # make sure the error is big on the one gaussian fit points, otherwise
        # drop them out

        #Manjit: TGray had set the value of chi2<0.5*chi1, but I am going to
        #change it to a different values.
    ...
    def plotIDStp(self, ts, writeFiles = False, plotErr = True, force = None,
        ...
        if plotErr:
            ...
            errorbar(xcoord[q] +
                dvshift,self.data[q,i],self.err[q,i],fmt='kx', ecolor =
                'red', capsize = 5, capthick=1.5)
        else:
            if style == 'step':
    ...
    # 		ylim(0,)
            ...
            if kTFit1.mask[i]:
                text(.62, .85, 'forced fit - no good',
                    horizontalalignment='left', verticalalignment='top',
                    transform = axis.transAxes, labelsize = 18)
            if printError:
        ...
        text(.05,.95, r"t = %3.1f $\mu$s" % (tp), horizontalalignment='left',
            verticalalignment='top', transform = axis.transAxes)
        ylim(0,)
        ylabel('Photocurrent (microA)',fontsize=14, weight='bold')
        xlabel('Velocity (km/s)',fontsize=14, weight='bold')
        tick_params(axis='both', direction='in', length=7, width =2, labelsize = 12)
        setp(axis.spines.values(), linewidth=2)
        title(self.shotname)
        show()
        if writeFiles:
            dirName = ssxutil.ssxPath('', 'output', self.runYear + '/' +
    ...
    def plotIDS(self, savepng = False, writeFiles = False, plotAll = False, maybePts = False,
        momPlot = False, ms = 4, showLegend = False, ymax = None):
        """Plot fitted IDS data as a function of time."""

        #      ms stands for marker size
        # ymax = stands for the maximum temperature value (Y-axis) upto which
        #the graph is going to be plotted
        # 	momPlot = False
        ...
        figs = {}
        # plot kTi
# original definition of f2 was this (by Tim Gray)
#        f2 = figure(2, **ssxdef.f)
        f2 = figure(2, figsize=(3.375, 3),dpi=300,facecolor='w',edgecolor='k')
        f2.subplots_adjust(top=0.92, bottom=0.16, left=0.15, right=0.95)
        figs['kTi'] = f2
        f2.clear()

#        fig=plt.figure(num=1,figsize=(5,3.7),dpi=250,facecolor='w',edgecolor='k')

        if momPlot:
            plot(t, self.kTMom, ls = ':', color = 'black', marker = 'o', mfc =
                'black', mec = 'black', label='2nd moment') #
     	#	plot(t, self.kTMom, 'k.', ms = ms) #

        errorbar(t, self.kTFit, self.kTErr, fmt='none', ecolor='black')
...
#        xlim(self.time[self.q['time'][0]], self.time[self.q['time'][-1]]+1)
    ...
    # find the maximum temperature
        ...
        if savepng:
            fName = ssxutil.ssxPath(self.shotname+'_Temperature.png')
            figs['kTi'].savefig(fName)
