...
@authors: Katie, Adam
Generates calibration LOOKUP TABLE for the 4x4 probe array.
...
def generateMatrix():
    ...
    savetxt(os.path.join(pth, 'calib-%s-4x4_3x3_no_switch.txt' % (date)) , calibration_matrix)

def generatelookup(num_probes = 16):
    """More precise than plotting becuase it doens't have to thin the times

        For every shot, find the calibration value, then average it
    """
    calibration_lookup= [[0] * 3 for i in range(num_probes)]
    ...
    savetxt(os.path.join(pth, 'calib-%s-4x4_lookup_5.txt' % (date)) , calibration_lookup)
