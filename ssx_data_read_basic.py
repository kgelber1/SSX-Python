...
from ssx_py_utils_basic import *
...
# basedir = '/Users/LBarbano/Desktop/Summer_2017/SSX_book'
basedir = os.getcwd()
...
# 		self.runSplit()
    ...
    def _read_data_file(self):
        ...
        if tmp[0] == '#':
            ...
            my_data.headerCols = tmp[1:].decode().strip().split('\t')
        ...
        if hasattr(self.rawdata, 'headerCols'):
...
class scope_data(ssx_data):
    def __init__(self,shotname,scope=None):
        ...
        if scope == 1 or '1' in scope:
            scope = '1'
        if scope == 2 or '2' in scope:
            scope = '2'
            # print('ye')
        if scope == 3 or '3' in scope:
            scope = '3'
...
# 			fileLen = fninfo.size
        ...
        if 'capturestats.txt' in self.info.keys():
...
# 			data[i-1, :] = a
        ...
        fp = t.extractfile(fn)
        s = fp.read()
        fcalib = array([float(x) for x in s.decode().split(',')])
    ...
    def _assignChannelNames(self):
        ...
        if 'channel-names.txt' in self.info.keys():
