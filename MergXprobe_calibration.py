...
def probe_calib(shot, probe_num, position_vector, dir):
    ...
    return ratio_x, ratio_y, ratio_z, currmax, helmB, normfactor
...
def getRatio(probe_num, position_vector, shot_range, dir, day ='050119r'):
    ...
    for shot in range(shot_range[0], shot_range[1]+1):
        ...
        x,y,z, currmax,helmB, norm_factor = probe_calib(day+str(shot), probe_num, position_vector,dir)
        ratio_x = ratio_x + x
        ratio_y = ratio_y + y
        ratio_z = ratio_z + z
        helm_B = [helm_B[i] + helmB[i] for i in len(helmB)]
...
def main():
    ...
    for probe_num in range(num_probes):
        ...
        calibration_matrix[counter] = [ratio_Bx, ratio_By, ratio_Bz, norm_factor]
        counter +=1
    ...
    pth = os.getcwd()
    date = '050119'
    print("Finished! File saved as calib-%s-4x4.txt in current working directory" %(date))
    savetxt(os.path.join(pth, 'calib-%s-4x4.txt' % (date)) , calibration_matrix)
